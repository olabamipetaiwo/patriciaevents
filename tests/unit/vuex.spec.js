...
describe("Vuex Store", () => {
  it("It set fetchingEvents to false after fetching events", () => {
    const eventItems = [
      {
        offers: [
          {
    ...
    expect(store.state.events.fetchingEvents).not.toBe(true);
    store.commit("FETCH_ALL_EVENTS", eventItems);
    expect(store.state.events.fetchingEvents).toBe(true);
  });
});
